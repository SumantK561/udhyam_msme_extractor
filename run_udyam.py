...
def main():
    ...
    logger.info(
        "Run ID=%s | States=%s | MaxWorkers=%s | BatchSize=10000 | TestState=%s | ResumeMode=%s",
        run_id,
        len(states),
        MAX_WORKERS,
        TEST_STATE or "ALL",
    ...
    logger.info(
        "Run ID=%s | States Requested=%s | Completed=%s | Failed=%s | Records=%s | Elapsed=%s",
        run_id,
        len(states),
        completed,
        failed,
    ...
    if failed:
        logger.warning(
            "Execution completed with %s failed State(s). Re-run to retry/resume.",
            failed,
        )
    else:
